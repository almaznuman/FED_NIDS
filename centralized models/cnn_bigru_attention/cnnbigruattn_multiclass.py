...
# -*- coding: utf-8 -*-
"""
PyTorch's implementation of the CNN+BiGRU model for multiclass intrusion detection on UNSW-NB15 dataset
"""
...
def load_data(train_path, test_path):
    ...
    return X_train, X_test, y_train_multi_class, y_test_multi_class
...
# CNN+BiGRU model with attention for multiclass classification
...
def evaluate_model(model, test_loader):
    """Evaluate the multiclass model on test data"""
    model.eval()
...
def plot_metrics(metrics):
    ...
    bars = plt.bar(categories, values, color='#ffc300')

    plt.xlabel('Metrics')
    plt.ylabel('Scores')
    plt.title('CNN + Bi-GRU with Attention - Multiclass Classification (PyTorch)')
    plt.ylim(0, 1.0)
    ...
    plt.tight_layout()
    plt.savefig('multiclass_model_metrics.png')
    plt.show()
...
def main():
    ...
    # Load and preprocess data
    X_train, X_test, y_train_multi_class, y_test_multi_class = load_data(train_path, test_path)
    ...
    batch_size = 32
    train_dataset = TensorDataset(X_train_tensor, y_train_multi_class_tensor)
    ...
    test_dataset = TensorDataset(X_test_tensor, y_test_multi_class_tensor)
    ...
    hidden_dim = 50
    model = CNNBiGRUMulticlassModel(input_dim, hidden_dim, num_classes, dropout=0.01).to(device)
    print(model)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(checkpoint_dir, 'multiclass_training_history.png'))
    plt.show()
    ...
    import json
    with open(os.path.join(checkpoint_dir, 'multiclass_evaluation_metrics.json'), 'w') as f:
