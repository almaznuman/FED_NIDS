...
# -*- coding: utf-8 -*-
"""
Hyperparameter tuning for CNN+BiGRU model using Optuna for multiclass classification
...
def load_data(train_path, test_path):
    """Load and preprocess the UNSW-NB15 dataset for multiclass classification"""
    ...
    scaler = MinMaxScaler()
    features_to_scale = data.columns.difference([categorical_target])
    data[features_to_scale] = scaler.fit_transform(data[features_to_scale])

    # Extract features and targets
    X = data.drop(columns=[categorical_target])
    y_multi_class = data[categorical_target]

    # Get the number of classes
    num_classes = len(y_multi_class.unique())
    ...
    # Split the data
    X_train, X_test, y_train_multi_class, y_test_multi_class = train_test_split(
        X_scaled, y_multi_class, test_size=0.2, random_state=42)

    # Reshape data for CNN (adding channel dimension)
    X_train = X_train.reshape(X_train.shape[0], X_train.shape[1], 1)
    X_test = X_test.reshape(X_test.shape[0], X_test.shape[1], 1)

    return X_train, X_test, y_train_multi_class, y_test_multi_class, num_classes


# CNN+BiGRU model for multiclass classification with tunable hyperparameters
class CNNBiGRUMulticlassModel(nn.Module):
    def __init__(self, input_dim, num_classes, cnn_filters=64, kernel_size=3, 
                 hidden_dim=50, fc_units=100, dropout=0.01,
                 bidirectional=True, num_layers=1):
        super(CNNBiGRUMulticlassModel, self).__init__()
        ...
        self.multiclass_output = nn.Linear(fc_units, num_classes)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, 
    ...
    """Train the PyTorch model for multiclass classification"""
    ...
    for epoch in range(num_epochs):
        ...
        val_precision = precision_score(val_targets_list, val_preds, average='macro')
        val_recall = recall_score(val_targets_list, val_preds, average='macro')
        val_f1 = f1_score(val_targets_list, val_preds, average='macro')
...
def objective(trial):
    ...
    if not hasattr(objective, "data_loaded"):
        ...
        # Load and preprocess data
        X_train, X_test, y_train_multiclass, y_test_multiclass, num_classes = load_data(train_path, test_path)
        
        # Convert numpy arrays to PyTorch tensors
        X_train_tensor = torch.FloatTensor(X_train)
        y_train_multiclass_tensor = torch.LongTensor(y_train_multiclass.values)
        
        X_test_tensor = torch.FloatTensor(X_test)
        y_test_multiclass_tensor = torch.LongTensor(y_test_multiclass.values)
        
        # Store data for reuse
        objective.X_train = X_train
        objective.X_test = X_test
        objective.X_train_tensor = X_train_tensor
        objective.y_train_multiclass_tensor = y_train_multiclass_tensor
        objective.X_test_tensor = X_test_tensor
        objective.y_test_multiclass_tensor = y_test_multiclass_tensor
    ...
    y_train_multiclass_tensor = objective.y_train_multiclass_tensor
    ...
    train_dataset = TensorDataset(X_train_tensor, y_train_multiclass_tensor)
...
def save_best_model(study, X_train, X_test, y_train_multiclass, y_test_multiclass, num_classes, output_dir="multiclass_model_hyperopt"):
    ...
    test_dataset = TensorDataset(X_test_tensor, y_test_multiclass_tensor)
    test_loader = DataLoader(test_dataset, batch_size=batch_size)
    
    # Initialize the model with best hyperparameters
    input_dim = X_train.shape[1]
    best_model = CNNBiGRUMulticlassModel(
        input_dim=input_dim,
        num_classes=num_classes,
    ...
    metrics, conf_matrix = evaluate_model(best_model, test_loader, num_classes)
...
def evaluate_model(model, test_loader, num_classes):
    """Evaluate the multiclass classification model on test data"""
    model.eval()
...
def plot_metrics(metrics, output_dir="multiclass_model_hyperopt"):
    """Plot the evaluation metrics for multiclass classification"""
    ...
    plt.xlabel('Metrics')
    plt.ylabel('Scores')
    plt.title('Lightweight CNN+BiGRU Model - Multiclass Classification')
    plt.ylim(0, 1.0)
...
def plot_optimization_history(study, output_dir="multiclass_model_hyperopt"):
...
def main():
    # Output directory
    output_dir = "../multiclass_model_hyperopt"
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    ...
    print("Starting hyperparameter optimization for lightweight CNN+BiGRU multiclass model...")
    
    # Create an Optuna study that maximizes the score
    study = optuna.create_study(
        direction="maximize",
        pruner=MedianPruner(n_startup_trials=5, n_warmup_steps=5),
        study_name="lightweight_cnn_bigru_multiclass"
    )
    ...
    best_model, metrics = save_best_model(
        study, X_train, X_test, y_train_multiclass, y_test_multiclass, num_classes, output_dir
    )
    ...
    original_model = CNNBiGRUMulticlassModel(input_dim=X_train.shape[1], num_classes=num_classes, hidden_dim=50)
