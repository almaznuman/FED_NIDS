...
# -*- coding: utf-8 -*-
"""
Example script to load and use the optimized lightweight CNN+BiGRU multiclass model
"""
...
import torch
import json
from cnnbigru_multiclass_optuna import CNNBiGRUMulticlassModel, load_data

# Set device
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
print(f"Using device: {device}")

def load_optimized_model(model_path='multiclass_model_hyperopt/best_model.pth'):
    ...
    return model, hyperparams, num_classes

def load_original_model(input_dim, num_classes, hidden_dim=50):
...
def compare_models(optimized_params, output_dir='multiclass_model_hyperopt'):
...
def predict_sample(model, input_tensor, num_classes):
...
def main():
    ...
    try:
        ...
        optimized_model, hyperparams, num_classes = load_optimized_model()
        
        # Load a few samples from the dataset for demonstration
        X_train, X_test, y_train, y_test, _ = load_data(train_path, test_path)
        input_dim = X_train.shape[1]
        
        # Load original model for comparison
        original_model = load_original_model(input_dim, num_classes)
        ...
        print(f"{'Sample':^8} | {'True Label':^10} | {'Original Pred':^12} | {'Optimized Pred':^14} | {'Orig Top Conf':^12} | {'Opt Top Conf':^12}")
        print("-" * 85)
        ...
        for i in range(num_samples):
            ...
            orig_pred, orig_conf = predict_sample(original_model, sample, num_classes)
            
            # Predict with optimized model
            opt_pred, opt_conf = predict_sample(optimized_model, sample, num_classes)
            
            # Get top confidence values
            orig_top_conf = np.max(orig_conf[0])
            opt_top_conf = np.max(opt_conf[0])
            
            # Print results
            print(f"{i:^8} | {true_labels[i]:^10} | {orig_pred[0]:^12} | {opt_pred[0]:^14} | {orig_top_conf:.4f} | {opt_top_conf:.4f}")
        ...
        # Print class-wise summary if we have a multiclass model
        if num_classes > 2:
            print("\nClass Distribution in Test Set:")
            for class_idx in range(num_classes):
                class_count = (y_test == class_idx).sum()
                print(f"Class {class_idx}: {class_count} samples ({class_count/len(y_test)*100:.2f}%)")
        
    except FileNotFoundError as e:
        print(f"Error: {e}")
        print("Please run the multiclass hyperparameter tuning script first.")
