"""BiGRU: A Flower / PyTorch app for multiclass intrusion detection."""
...
class CNNBiGRUMulticlassModel(nn.Module):
    def __init__(self, input_dim, hidden_dim=50, num_classes=10, dropout=0.01):
...
def load_model(learning_rate: float = 0.001, num_classes: int = 10):
    """Create and initialize a new CNN+BiGRU model for multiclass classification.
    ...
    model = CNNBiGRUMulticlassModel(input_dim=len(features), num_classes=num_classes)
...
def preprocess_data(data):
    ...
    for col in data.select_dtypes(include=['object']).columns:
        if col != 'attack_cat':  # Don't encode the target column yet
            data[col] = label_encoder.fit_transform(data[col])

    # Apply MinMax normalization to all features
    scaler = MinMaxScaler()
    features_to_scale = data.columns.difference(['attack_cat'])
...
def load_data(partition_id, num_partitions):
    ...
    # First, ensure we have enough samples for each class
    unique_classes, class_counts = np.unique(y, return_counts=True)
    min_samples_per_class = 2  # Minimum samples needed for stratification
    
    # If any class has fewer than min_samples_per_class samples, use random split
    ...
    y_train_tensor = torch.LongTensor(y_train)  # LongTensor for multiclass
    X_test_tensor = torch.FloatTensor(X_test)
    y_test_tensor = torch.LongTensor(y_test)  # LongTensor for multiclass
...
def load_cen_data():
    ...
    y_tensor = torch.LongTensor(y)  # Changed to LongTensor for multiclass classification
...
def train(net, trainloader, epochs, optimizer, criterion, device):
    ...
    for epoch in range(epochs):
        ...
        for inputs, targets in trainloader:
            ...
            targets = targets.long().to(device)  # Convert targets to long for CrossEntropyLoss
