"""Client selection strategies combining loss-based and gradient similarity approaches."""
...
from baselines.cifar_10_baseline.task import create_run_dir, Net, set_weights

# Define a project name for logging (if using W&B)
# PROJECT_NAME = "Diversity Testing"
PROJECT_NAME = "FEDAVG vs Reliability Index Runs Cifar runs"
...
class LossBasedSelectionStrategy(FedAvg):
    ...
    def _update_best_acc(self, round_number, accuracy, parameters):
        ...
        if accuracy > self.best_acc_so_far:
            ...
            file_name = f"model_state_acc_{accuracy}_round_{round_number}.pth"
    ...
    def evaluate(self, server_round: int, parameters: Parameters) -> tuple[float, dict[str, Scalar]]:
        ...
        acc = metrics.get("centralized_accuracy", 0.0)
